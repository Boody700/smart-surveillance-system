...
PHONE_MODEL_PATH = os.path.join(ROOT_DIR, "Phone_best_.pt")
...
if os.path.exists(PHONE_MODEL_PATH):
    ...
    YOLO26_TEST_MODEL = "yolo26n.pt"   # testing if stock YOLO26 beats yolo12n on phones
    print(f"[INFO] Custom phone model not found — testing stock {YOLO26_TEST_MODEL} on class 67")
    phone_model = YOLO(YOLO26_TEST_MODEL)
...
# Phone persistence filtering — absorbs single-frame false positives/flicker.
# A phone must be seen near the SAME person for N consecutive seconds before
# it's logged as a confirmed "phone usage" event.
# NOTE: this is bbox-proximity based for now. Upgrade to face-proximity once
# face linking/recognition is solid (flagged for later).
PHONE_PERSISTENCE_SECONDS = 5     # tune this — start moderate, adjust after testing
phone_streak_frames = {}          # clean_id -> consecutive frames phone seen near them
phone_confirmed     = {}          # clean_id -> bool, already logged this streak (avoid duplicate spam)
...
while True:
    ...
    if phone_model is not None:
        is_custom_phone_model = os.path.exists(PHONE_MODEL_PATH)
        predict_kwargs = dict(
            conf=0.25 if is_custom_phone_model else 0.15,
            iou=0.30,
            imgsz=960,
            verbose=False
        )
        if not is_custom_phone_model:
            predict_kwargs["classes"] = [67]   # stock COCO model — filter to phone class only

        res_phones = phone_model.predict(frame, **predict_kwargs)
    else:
    ...
    seen_this_frame = set()   # clean_ids that have a phone near them THIS frame
    ...
    if phone_frame.boxes is not None and len(phone_frame.boxes):
        ...
        for box, conf in zip(ph_boxes, ph_confs):
            ...
            # Raw detection — always logged, useful for debugging/tuning conf later
            cursor.execute("""
                INSERT INTO events
                    (timestamp, person_id, event_type, confidence,
                     bbox_x1, bbox_y1, bbox_x2, bbox_y2)
            ...
            # Persistence tracking — only counts toward a confirmed violation
            # if a person owns it (proximity-based for now; face-proximity later)
            box_color = (0, 255, 255)   # default yellow — not yet confirmed
            if owner_id is not None:
                seen_this_frame.add(owner_id)
                phone_streak_frames[owner_id] = phone_streak_frames.get(owner_id, 0) + 1

                streak_seconds = phone_streak_frames[owner_id] / fps
                if streak_seconds >= PHONE_PERSISTENCE_SECONDS:
                    box_color = (0, 0, 255)   # red — confirmed usage
                    if not phone_confirmed.get(owner_id, False):
                        cursor.execute("""
                            INSERT INTO events
                                (timestamp, person_id, event_type, confidence,
                                 bbox_x1, bbox_y1, bbox_x2, bbox_y2, duration_seconds)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (round(current_ts, 3), owner_id, "violation_phone_usage",
                              round(conf, 4), x1, y1, x2, y2, round(streak_seconds, 2)))
                        phone_confirmed[owner_id] = True
                        print(f"[VIOLATION] Person {owner_id} — phone usage confirmed "
                              f"({streak_seconds:.1f}s)")
    ...
    # Reset streaks for anyone who didn't have a phone near them this frame
    for cid in list(phone_streak_frames.keys()):
        if cid not in seen_this_frame:
            phone_streak_frames[cid] = 0
            phone_confirmed[cid] = False   # allow re-confirmation on next streak
