...
output_path = os.path.join(output_dir, "output jimmy.mp4")
...
# --- TRACKING REGISTRIES ---
raw_id_counters = {} # How many consecutive frames each raw ID has survived
last_frame_ids = set() # IDs seen in the previous frame (for continuity check)
...
while True:
    ...
    # Agent 2 uses this to measure how long someone has been away from their desk.
    current_timestamp = frame_count / fps

    # Run ByteTrack tracking on this frame
    results = model.track(
        frame,
        persist=True,
        tracker="agent1_tracking/custom_tracker.yaml",
        classes=[0], # Only detect people (class 0 in COCO dataset)
        conf=0.20, # Lower confidence threshold to catch more distant people
        iou=0.20, # Aggressively merge overlapping boxes
        imgsz=1536, # Higher resolution scan
        verbose=False
    )
    ...
    if frame_results.boxes.id is not None:
        ...
        # --- PHASE 1: UPDATE PROBATION COUNTERS ---
        for raw_id in raw_ids:
            if raw_id in last_frame_ids:
                raw_id_counters[raw_id] = raw_id_counters.get(raw_id, 0) + 1
            else:
                # ID disappeared last frame and came back — reset its probation
                raw_id_counters[raw_id] = 1

            # Graduate from probation and assign a clean sequential ID
            if raw_id_counters[raw_id] >= MIN_FRAMES_TO_CONFIRM and raw_id not in id_registry:
                id_registry[raw_id] = next_clean_id
                next_clean_id += 1
                print(f"[NEW PERSON] Person ID {id_registry[raw_id]} confirmed at "
                      f"frame {frame_count} ({current_timestamp:.1f}s)")
        ...
        for raw_id, box, conf in zip(raw_ids, boxes, confidences):
            if raw_id in id_registry:
                ...
                cursor.execute("""
                    INSERT INTO events
                    (timestamp, person_id, event_type, confidence, bbox_x1, bbox_y1, bbox_x2, bbox_y2)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
