...
# ═══════════════════════════════════════════════════════
# TAB 4 — CHAT WITH YOUR FOOTAGE
# ═══════════════════════════════════════════════════════
with tab4:
    st.markdown("#### Chat with this session")
    st.markdown(
        '<p style="color:#5b6b82;font-size:0.85rem;line-height:1.6;">'
        'Ask questions about the people and violations tracked in this run — '
        'e.g. "who was AFK the longest?" or "did anyone use their phone?". '
        'Answers are grounded only in this session\'s own data, using the '
        'same local model as the PDF report narrative.</p>',
        unsafe_allow_html=True
    )

    people, detections, violations_count = get_stats()
    if detections == 0:
        st.info("No session data yet — run Detection (Step 1) first, so there's something to chat about.")
    else:
        ...
        if question:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking…"):
                    # Last 10 turns of prior conversation for follow-up
                    # continuity - the DB summary itself is rebuilt fresh
                    # every call inside chat_assistant.ask().
                    prior_turns = st.session_state["chat_history"][-11:-1]
                    answer = chat_assistant.ask(question, DATABASE_PATH, history=prior_turns)
                st.markdown(answer)
